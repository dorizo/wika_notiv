...
class Bapmoverveiw extends StatefulWidget {
  final String kategoripage;
  const Bapmoverveiw({Key key, this.kategoripage}) : super(key: key);

  @override
  _BapboverviewState createState() => _BapboverviewState();
}

class _BapboverviewState extends State<Bapmoverveiw> {
  int page = 0;
  DateTime _selectedDate;
  TextEditingController _textEditingController = TextEditingController();
  ScrollController _controller = ScrollController();
  bool isLoading = false;
  Future<List<Bapmmodel>> x;
  List<Bapmmodel> pagging = [];
    ...
    Session().sessionku().then((value) {
      setState(() {
        ...
        x = Internethttp().bapminternt(
            ...
            sessiondata.kdJbt);
        Internethttp()
            .bapminternt(page.toString(), "20", kodeSppdn, tanggalSppdn, status,
  ...
  Widget cardCatalog(Bapmmodel product) {
    return Container(
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            product.nOBAPM != null ? product.nOBAPM : "",
                          ...
                          Text(
                            product.tGLBAPM != null ? product.tGLBAPM : "",
                            style: TextStyle(fontWeight: FontWeight.bold),
                            textAlign: TextAlign.left,
                          ),
                          SizedBox(
                          ...
                          Text(
                            product.tGLSPB != null ? product.tGLSPB : "",
                            style: TextStyle(fontWeight: FontWeight.normal),
                            textAlign: TextAlign.left,
                          ),
                          // SizedBox(
                          ...
                          Text(
                            product.nAMAPAT != null
                                ? product.nAMAPAT + " -> " + product.nAMAPATTO
                                : "",
                            style: TextStyle(fontWeight: FontWeight.normal),
                            textAlign: TextAlign.left,
                          ),
                    ...
                    Column(
                      children: [
                        FlatButton(
                            minWidth: 110,
                            onPressed: () {
                              Navigator.pushNamed(context, "/bapmdetail",
                                  arguments: product);
                            },
                            color: Colors.blue,
                            child: Row(
  ...
  Widget d(String ssppdn, String tanggal, String statusinet) {
    Internethttp()
        .bapminternt(page.toString(), "20", ssppdn, tanggal, statusinet,
            sessiondata.kdPat, widget.kategoripage, sessiondata.kdJbt)
        .then((value) {
      setState(() {
        page = 0;
